...
import {activateZone, getFixturesForZone, isZoneInactive, setZoneChangeCB, zonesMap} from './zones';
...
for (const [k, v] of Object.entries(zonesMap)) {
  vermuthSender.addNamedChase(k, v.fixtures)
}
...
function sensorActivity(name: string, hasChange: boolean, v) {
  console.log(hasChange ? 'change' : 'update', name, v);
...
function processMsg(msg: any, time: any, info: any) {
